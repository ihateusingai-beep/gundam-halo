"""Voice REST config endpoints — `/voice/status`, `/voice/config` GET + PUT.

Sprint 32 P1.1: extracted from `app/api/voice_ws.py`. The REST
endpoints (4 total) live here so the WS route file
(`ws_protocol.py`) stays focused on the WebSocket control
loop.

Sprint 56 R4: extracted from `app/api/voice_config_api.py`
(990 LoC monolith). This module owns:
  - GET /voice/status         — voice layer status
  - GET /voice/config         — current config + restart flags
  - GET /voice/eval-results   — held-out eval trend for the
                                dashboard (Sprint 39)
  - PUT /voice/config         — update config in-memory + persist

The eval/finetune/corpora endpoints moved to sibling files
(`eval_jobs.py`, `corpora.py`).

The endpoints register against the SHARED `router` object
imported from `ws_protocol.py` (FastAPI allows multiple
modules to add routes to the same `APIRouter` instance —
FastAPI's `include_router` walks the route table at app
startup, so the order of registration doesn't matter).
...
async def put_voice_config(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    # untouched. If present, the value must be one of the
    # accepted enum values (we validate against the ASR
    # registry so a typo at write-time doesn't surface as a
    # cryptic 500 when the user reconnects the voice WS).
    new_asr_backend: str | None = None
    new_asr_corrector: str | None = None
    if "asr_backend" in payload and payload["asr_backend"] is not None:
        candidate = payload["asr_backend"]
        if not isinstance(candidate, str):
            raise HTTPException(
                status_code=400,
                detail="`asr_backend` must be a string",
            )
        # Sprint 56 R4: validate against the ASR registry
        # (the authoritative source of truth for "what backends
        # are wired in this build"). The `whisper_hf` backend
        # (Sprint 23) and `yuesub` (Sprint 17b) were missing
        # from the `ALLOWED_ASR_BACKENDS` setup_state constant;
        # the registry covers all of them.
        from app.core.registry import AsrRegistry
        if not AsrRegistry.contains(candidate):
            available = sorted(name for name, _ in AsrRegistry.items())
            raise HTTPException(
                status_code=400,
                detail=(
                    f"`asr_backend` {candidate!r} is not a registered ASR backend. "
                    f"Available: {available}."
                ),
            )
        new_asr_backend = candidate
    if "asr_corrector" in payload and payload["asr_corrector"] is not None:
        candidate = payload["asr_corrector"]
        if not isinstance(candidate, str):
            raise HTTPException(
                status_code=400,
                detail="`asr_corrector` must be a string",
            )
        # Sprint 56 R4: corrector allowlist. The valid set is
        # yuesub-only (whisper_local doesn't use a corrector),
        # so a whis_per_local user accidentally setting
        # `asr_corrector = "bert"` would 500 on the next voice
        # turn. Hard-coded here until a CorrectorRegistry is
        # added to app/core/registry.py (deferred — no
        # corrector swap path exists in the UI today).
        allowed_correctors = sorted({"bert", "opencc", "none"})
        if candidate not in allowed_correctors:
            raise HTTPException(
                status_code=400,
                detail=(
                    f"`asr_corrector` {candidate!r} is not a valid corrector. "
                    f"Available: {allowed_correctors}."
                ),
            )
        new_asr_corrector = candidate

    # ---- Apply in-memory ----
    cfg = get_config()
    voice_cfg = cfg.voice
    voice_cfg.wake_phrases = normalized
    voice_cfg.strict_wake_phrase = new_strict
    if new_always_on_mic is not None:
        voice_cfg.always_on_mic = new_always_on_mic
    asr_changed = False
    if new_asr_backend is not None and new_asr_backend != voice_cfg.asr.backend:
        voice_cfg.asr.backend = new_asr_backend
        asr_changed = True
    if (
        new_asr_corrector is not None
        and new_asr_corrector != voice_cfg.asr.corrector
    ):
        voice_cfg.asr.corrector = new_asr_corrector
        asr_changed = True
    # If the asr fields changed, flag the dashboard to surface
    # a "restart required" banner. The flag is cleared by any
    # subsequent PUT that doesn't change either field.
    if asr_changed:
        set_restart_required(True)
        # Sprint 56 R4: original code called
        # `schedule_restart_if_needed()` with no args — but the
        # function's signature is keyword-only `restart_required`
        # + `reason` (per app/api/restart_handler.py:56). The
        # call was broken at the PUT path since Sprint 19b; no
        # test caught it because no fixture actually triggered
        # the asr-changed branch. Pass the right args here.
        schedule_restart_if_needed(
            restart_required=True,
            reason="asr_backend_or_corrector_changed",
        )
    else:
        # Sprint 18: a non-asr PUT also clears any pending
        # restart flag (e.g. user dismissed the banner by
        # saving the form again without changing the asr).
        set_restart_required(False)
        # Sprint 56 R4: also clear the scheduled-restart flag
        # so `is_restart_scheduled()` returns False in the
        # response. Without this, the test
        # `test_put_wake_phrases_only_returns_restart_scheduled_false`
        # sees a stale True from a previous test that scheduled
        # a restart. The helper sets both flags in one call.
        from app.core.restart import _set_restart_scheduled
        _set_restart_scheduled(False, reason="non_asr_put")

    # ---- Persist to config.toml ----
    # Use cfg.home (the runtime-resolved home from get_config())
    # so the write goes to the same directory the load came
    # from — important for tests that monkeypatch HALO_HOME
    # before the PUT.
    from app.core import config as _config_mod
    config_path = Path(_config_mod.get_config().home) / "config.toml"
    persisted = False
    try:
        doc = read_doc(config_path)
        # Each `update_section_key` call auto-creates the target
        # section if missing (see app.core.toml_doc for the
        # contract).
        ...
        if new_asr_backend is not None:
            update_section_key(doc, "voice.asr", "backend", new_asr_backend)
        ...
        logger.error(f"voice config TOML write failed: {e}")

    # ---- Build response ----
    # We always re-read the in-memory cfg so the response matches
    # what the next GET will return (catches the case where the
    # user sent a value that got coerced to a different in-memory
    # value — the only known one today is whitespace-trim).
    cfg = get_config().voice
    # Sprint 56 R4: include the live restart countdown in the
    # response so the dashboard can show a "Restart in 5s" toast
    # without a second round-trip (Sprint 41 contract).
    from app.core.restart import (
        get_restart_countdown_s,
        is_restart_scheduled,
    )
