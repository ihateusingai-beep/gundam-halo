...
import { BACKGROUNDS, resolveBgAsset } from "../constants";
    ...
    <HudCard>
      ...
      <Section title="Cockpit Background">
        <p className="text-xs text-[var(--text-muted)] mb-3 font-mono">
          Default: pure CSS hex grid. Optional: per-theme hero JPG.
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
          {BACKGROUNDS.map((b) => {
            // Sprint 58: per-theme thumb. Picking SEED shows
            // bg-seed-core-01.jpg; picking NT-D shows the
            // legacy unicorn-core-XX. Unknown themes fall back
            // to unicorn.
            const thumb = resolveBgAsset(b.id, theme);
            return (
              <button
                key={b.id}
                onClick={() => {
